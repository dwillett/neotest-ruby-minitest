...
describe("positions.discover_positions", function()
  ...
  async.it("discovers all test in a rails system test", function()
    local pos = positions.discover_positions(minitest("system_case_test.rb"))
    assert.are.same("system_case_test.rb", file(pos).name)
    assert.are.same("BareMinimumSystemTest", namespace(pos).name)
    assert.are.same("plain visit", test(pos).name)
  end)

  async.it("discovers all test in a rails custom system test", function()
    local pos = positions.discover_positions(minitest("test_case_test.rb"))
    assert.are.same("test_case_test.rb", file(pos).name)
    assert.are.same("Some::Custom", namespace(pos).name)
    assert.are.same("rails test case", test(pos).name)
  end)

  async.it("discovers all test in a rails test with modules", function()
    local pos = positions.discover_positions(minitest("modules_everywhere_test.rb"))
    assert.are.same("modules_everywhere_test.rb", file(pos).name)
    assert.are.same("MyModule::MyTest", namespace(pos).name)
    assert.are.same("my module test", test(pos).name)
  end)
end)
